...
updated: 2026-02-14 13:22:14 +0900
title: "리액트에서 데이터는 어떻게 가져와야 할까"
tags: ["react", "frontend", "data-fetching"]
---
...
리액트에서 널리 사용되는 HTTP 클라이언트 라이브러리에서 제공하는 [[react-hooks-rules|hook]]을 사용하는 것이 더 일반적인 것 같지만, `fetch`나 `axios`를 사용한다면 `useEffect`를 활용해서 데이터를 가져올 수 있다. 초기 렌더링 후 시점에만 데이터를 요청하도록 의존성 배열을 비워둔다.

### 공통 부모에서 가져오기

데이터에 관심을 가지는 여러 컴포넌트가 있다고 가정했을 때 공통 부모 컴포넌트에서 가져오는 방식이다.

![공통_부모에서_가져오기 | size='480'](./assets/react-data-common_parent.png)
...
![로딩_인디케이터_위치에서_가져오기_1 | size='480'](./assets/react-data-loading-indicator-1.png)
...
![로딩_인디케이터_위치에서_가져오기_2 | size='480'](./assets/react-data-loading-indicator-2.png)
...
![로딩_인디케이터_위치에서_가져오기_3 | size='480'](./assets/react-data-loading-indicator-3.png)
